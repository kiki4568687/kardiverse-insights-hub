  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 min-h-screen p-6">
      {/* Header */}
        ...
        <div>
          <h1 className="text-3xl font-bold text-white neon-text">Tab 5 - Escrow & Payments</h1>
          <p className="text-lg text-cyan-300 mt-2 text-glow">M-Pesa & SEPA Releases, Balance, Logs</p>
          <div className="flex items-center gap-2 mt-2">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse icon-glow"></div>
            <span className="text-sm text-green-400 font-medium text-glow">Last Sync - NOC verified ✓</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-card border-border card-glow border border-cyan-400/20">
          <CardHeader>
            <CardTitle className="text-xl text-white neon-text">Sponsor Accounts</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <TableBody>
                  {sponsorAccounts.map((account, index) => (
                    <TableRow key={index} className="border-slate-700 hover:bg-slate-700/50 glow-on-hover">
                      <TableCell className="text-white font-medium text-glow">{account.sponsor}</TableCell>
                      <TableCell className="text-white">{account.currency}</TableCell>
                      <TableCell className="text-white text-glow">{account.escrowBalance}</TableCell>
                      <TableCell className="text-white">{account.lastAudit}</TableCell>
                      <TableCell>
                        <Button
                          variant="outline"
                          size="sm"
                          className={`${getStatusColor(account.statusType)} text-xs button-glow glow-on-hover`}
                          onClick={() => {
        ...
        <Card className="bg-card border-border card-glow border border-purple-400/20">
          <CardHeader>
            <CardTitle className="text-xl text-white neon-text">Escrow Overview</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-slate-300 text-glow">Growth Trend</h4>
                <div className="h-24 flex items-end justify-between gap-1">
                  {[20, 40, 60, 80].map((height, index) => (
                    <div
                      key={index}
                      className="bg-blue-400 rounded-t card-glow glow-on-hover"
              ...
              {/* Donut Chart */}
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-slate-300 text-glow">Currency Distribution</h4>
                <div className="h-24 flex items-center justify-center">
                  <div className="relative w-16 h-16">
                    <svg className="w-16 h-16 transform -rotate-90" viewBox="0 0 32 32" style={{ filter: 'drop-shadow(0 0 8px rgba(0, 255, 255, 0.5))' }}>
                      <circle
                        cx="16"
                        cy="16"
                        r="14"
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-xs font-bold text-white text-glow">83%</span>
                    </div>
              </div>
                </div>
                <p className="text-xs text-center text-slate-400 text-glow">EUR</p>
              </div>
              </div>

            {/* Payment Timeline */}
            <div>
              <h4 className="text-sm font-medium text-slate-300 mb-3 text-glow">Payment Timeline</h4>
              <div className="space-y-2">
                {paymentTimeline.map((payment, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-slate-700/50 rounded text-xs glow-on-hover card-glow">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 bg-blue-400 rounded-full icon-glow"></div>
                      <span className="text-white text-glow">{payment.date}</span>
                    </div>
                    <div className="text-slate-300">
                      <div className="font-mono text-glow">{payment.transactionId}</div>
              ...
              <Button
                ...
                className="w-full mt-3 border-cyan-400 text-cyan-400 hover:bg-cyan-400/10 button-glow glow-on-hover"
              >
                <Eye className="h-4 w-4 mr-2 icon-glow" />
                View Bank Statement
              </Button>
            </div>
          </CardContent>
      ...
      <Card className="bg-card border-border card-glow border border-green-400/20">
        <CardHeader>
          <CardTitle className="text-xl text-white neon-text">Verification Steps</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            {verificationSteps.map((step, index) => (
              <div key={index} className="flex items-center gap-3 p-3 bg-slate-700/50 rounded glow-on-hover card-glow">
                <div className="flex items-center gap-2">
                  {getStepStatusIcon(step.status)}
                  <span className="text-white font-medium text-glow">{step.step}.</span>
                </div>
                <span className={`text-sm text-glow ${getStepStatusColor(step.status)}`}>
                  {step.text}
                </span>
              </div>
            ))}
      ...
      <div className="flex gap-4">
        ...
        <Button
          ...
          className="flex-1 border-cyan-400 text-cyan-400 hover:bg-cyan-400/10 button-glow glow-on-hover"
        >
          <FileText className="h-4 w-4 mr-2 icon-glow" />
          Generate Ledger
        </Button>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4 text-xs">
            <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-green-500 rounded-full icon-glow"></div>
            <span className="text-slate-300 text-glow">Verified</span>
              </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-orange-500 rounded-full icon-glow"></div>
            <span className="text-slate-300 text-glow">Pending</span>
            </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-red-500 rounded-full icon-glow"></div>
            <span className="text-slate-300 text-glow">Flagged</span>
              </div>
            </div>
        ...
        >
          <span className="text-glow">Export All</span>
          <ArrowRight className="h-4 w-4 icon-glow" />
          <span className="text-slate-300 text-glow">Bookkeeper Dashboard</span>
        </button>
      </div>
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl neon-text">
              <Receipt className="h-5 w-5 icon-glow" />
              Bank Statement - Kardiverse Escrow Account
            </DialogTitle>
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              <div className="flex items-center gap-2">
                <Calendar className="h-4 w-4 icon-glow" />
                <span className="text-glow">Statement Period: Oct 12, 2025 - Oct 17, 2025</span>
              </div>
              <div className="flex items-center gap-2">
                <Shield className="h-4 w-4 icon-glow" />
                <span className="text-glow">Account: KARDIVERSE-ESCROW-001</span>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <Card className="bg-card border-border card-glow border border-green-400/20">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm text-slate-300 text-glow">Current Balance</span>
                  </div>
                  <div className="text-2xl font-bold text-white text-glow">€1,450,000.00</div>
                  <div className="text-xs text-green-400 text-glow">+€450,000.00 this period</div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-card border-border card-glow border border-cyan-400/20">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <CreditCard className="h-4 w-4 text-cyan-400 icon-glow" />
                    <span className="text-sm text-slate-300 text-glow">Total Credits</span>
                  </div>
                  <div className="text-2xl font-bold text-white text-glow">€1,250,000.00</div>
                  <div className="text-xs text-cyan-400 text-glow">6 transactions</div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-card border-border card-glow border border-orange-400/20">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Banknote className="h-4 w-4 text-orange-400 icon-glow" />
                    <span className="text-sm text-slate-300 text-glow">Total Debits</span>
                  </div>
                  <div className="text-2xl font-bold text-white text-glow">€300,000.00</div>
                  <div className="text-xs text-orange-400 text-glow">3 transactions</div>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="flex gap-3">
              ...
              >
                <X className="h-4 w-4 mr-2 icon-glow" />
                Close
              </Button>
            </div>
          </div>
